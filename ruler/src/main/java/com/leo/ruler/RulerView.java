...
public class RulerView extends View {
    ...
    private VelocityTracker velocityTracker = VelocityTracker.obtain();// 追踪手指滑动速度
    ...
    public RulerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        attrSet(attrs, defStyleAttr);
        init();
    }

    private void attrSet(AttributeSet attrs, int defStyleAttr) {
        if (null == attrs) {
            return;
        }
        TypedArray a = getContext().getTheme().
                obtainStyledAttributes(attrs, R.styleable.RulerView, defStyleAttr, 0);
        ...
        bgRoundDp = a.getDimensionPixelSize(R.styleable.RulerView_bgRoundDp, bgRoundDp);
        a.recycle();
    }
    ...
    private void drawBg(Canvas canvas) {
        ...
        canvas.drawRoundRect(bgRect, bgRoundDp, bgRoundDp, bgPaint);
    }
    ...
    private void autoVelocityScroll(int xVelocity) {
        //惯性滑动的代码,速率和滑动距离,以及滑动时间需要控制的很好,这里是经过N次测试调节出来的惯性滑动
